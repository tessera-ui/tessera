...
impl ScrollState {
    ...
    pub fn with_sensitivity(
        viewport_size: PxPosition,
        content_size: PxPosition,
        sensitivity: f32,
    ) -> Self {
    ...
    pub fn handle_scroll_event(&mut self, event: &ScrollEventType) {
        ...
        // 更新偏移量并应用边界约束
        self.offset.x =
            Px((self.offset.x.0 - delta_x.0).clamp(self.bounds.min.x.0, self.bounds.max.x.0));
        self.offset.y =
            Px((self.offset.y.0 - delta_y.0).clamp(self.bounds.min.y.0, self.bounds.max.y.0));
    ...
    pub fn clamp_offset_to_bounds(&mut self) {
        self.offset.x = Px(self
            .offset
            .x
            .0
            .clamp(self.bounds.min.x.0, self.bounds.max.x.0));
        self.offset.y = Px(self
            .offset
            .y
            .0
            .clamp(self.bounds.min.y.0, self.bounds.max.y.0));
    }
...
impl ScrollBounds {
    ...
    pub fn contains(&self, position: PxPosition) -> bool {
        position.x.0 >= self.min.x.0
            && position.x.0 <= self.max.x.0
            && position.y.0 >= self.min.y.0
            && position.y.0 <= self.max.y.0
    }
}
...
mod tests {
    ...
    fn test_content_rect() {
        let rect1 = ContentRect {
            x: 10,
            y: 10,
            width: 100,
            height: 100,
        };
        let rect2 = ContentRect {
            x: 50,
            y: 50,
            width: 100,
            height: 100,
