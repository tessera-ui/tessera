...
pub enum Error {
    ...
    Processing(Box<ProcessingErrorContext>),
}

#[derive(Debug)]
pub struct ProcessingErrorContext {
    src: PathBuf,
    dest: PathBuf,
...
impl Reportable for Error {
    fn report(&self) -> Report {
        match self {
            ...
            Self::Processing(context) => Report::error(
                format!(
...
pub fn generate(
    ...
    for pack in pack_chain {
        ...
        bike.filter_and_process(pack, root, |_| (), filter.fun())
            .map_err(|cause| {
                Error::Processing(Box::new(ProcessingErrorContext {
                    src: pack.to_owned(),
                    dest: root.to_owned(),
                    cause,
                }))
