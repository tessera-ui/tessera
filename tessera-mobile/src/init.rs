...
impl Reportable for Error {
    fn report(&self) -> Report {
        match self {
            ...
            Self::DotFirstInitWriteFailed { path, cause } => Report::error(
            ...
            Self::DotFirstInitDeleteFailed { path, cause } => Report::action_request(
                format!(
                    "Failed to delete first init dot file {path:?}; the project generated successfully, but `cargo mobile init` will have unexpected results unless you manually delete this file!"
                ),
                cause,
            ),
            Self::OpenInEditorFailed(err) => Report::error(
                "Failed to open project in editor (your project generated successfully though, so no worries!)",
                err,
            ),
        }
    }
...
pub fn exec(
    ...
    project::generate(&config, &bike, &filter, submodule_commit)
    ...
    if metadata.android().supported() {
        match android::env::Env::new() {
            Ok(env) => android::project::generate(
                config.android(),
                metadata.android(),
                &env,
                &bike,
