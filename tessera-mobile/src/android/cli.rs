...
use crate::{
    android::{
        DEFAULT_ACTIVITY, NAME, aab, adb, apk,
...
pub enum Error {
    ...
    DevicePromptFailed(Box<PromptError<adb::device_list::Error>>),
    TargetInvalid(Box<TargetInvalid>),
    ConfigFailed(Box<LoadOrGenError>),
    MetadataFailed(Box<metadata::Error>),
    Unsupported,
    ProjectDirAbsent { project_dir: PathBuf },
    OpenFailed(Box<os::OpenFileError>),
    CheckFailed(Box<CompileLibError>),
    BuildFailed(Box<BuildError>),
    RunFailed(Box<RunError>),
    StacktraceFailed(Box<StacktraceError>),
    ListFailed(Box<adb::device_list::Error>),
    ApkError(Box<apk::ApkError>),
    AabError(Box<aab::AabError>),
}
    ...
    fn report(&self) -> Report {
        match self {
            ...
            Self::Unsupported => Report::error(
                ...
                "If your project should support Android, modify your Cargo.toml, then run `cargo mobile init` and try again.",
            ),
            Self::ProjectDirAbsent { project_dir } => Report::action_request(
                "Please run `cargo mobile init` and try again!",
                format!("Android Studio project directory {project_dir:?} doesn't exist."),
            ),
    ...
    fn exec(self, wrapper: &TextWrapper) -> Result<(), Self::Report> {
        ...
        fn with_config(
            ...
            let (config, _origin) = OmniConfig::load_or_gen(".", non_interactive, wrapper)
                .map_err(|err| Error::ConfigFailed(Box::new(err)))?;
            let metadata = OmniMetadata::load(config.app().root_dir())
                .map_err(|err| Error::MetadataFailed(Box::new(err)))?;
        ...
        fn open_in_android_studio(config: &Config, env: &Env) -> Result<(), Error> {
            os::open_file_with("Android Studio", config.project_dir(), &env.base)
                .map_err(|err| Error::OpenFailed(Box::new(err)))
        }
            ...
            if targets.is_empty() {
                ...
                for t in targets {
                    let target = Target::for_name(&t)
                        ...
                        .map_err(|err| Error::TargetInvalid(Box::new(err)))?;
                    outs.push(target);
                }
                Ok(outs)
            }
        ...
        match command {
                    ...
                    call_for_targets_with_fallback(
                        ...
                        |target: &Target| {
                            target
                                ...
                                .map_err(|err| Error::CheckFailed(Box::new(err)))
                        },
                    )
                    .map_err(|err| Error::TargetInvalid(Box::new(err)))?
                })
                ...
                call_for_targets_with_fallback(
                    ...
                    |target: &Target| {
                        target
                            ...
                            .map_err(|err| Error::BuildFailed(Box::new(err)))
                    },
                )
                .map_err(|err| Error::TargetInvalid(Box::new(err)))?
            }),
            Command::Run {
                ...
                ensure_init(config)?;
                device_prompt(env)
                    .map_err(|err| Error::DevicePromptFailed(Box::new(err)))?
                    .run(
                        config,
                        env,
                        noise_level,
                    ...
                    .map_err(|err| Error::RunFailed(Box::new(err)))
            }),
            Command::Stacktrace => with_config(non_interactive, wrapper, |config, _, env| {
                ...
                device_prompt(env)
                    ...
                    .map_err(|err| Error::StacktraceFailed(Box::new(err)))
            }),
            Command::List => with_config(non_interactive, wrapper, |_, _, env| {
                adb::device_list(env)
                    .map_err(|err| Error::ListFailed(Box::new(err)))
                    .map(|device_list| {
            ...
            Command::Apk { cmd } => match cmd {
                ApkSubcommand::Build {
                    ...
                    .map_err(|err| Error::ApkError(Box::new(err)))
                }),
            },
            Command::Aab { cmd } => match cmd {
                AabSubcommand::Build {
                    ...
                    .map_err(|err| Error::AabError(Box::new(err)))
                }),
            },
        }
    }
