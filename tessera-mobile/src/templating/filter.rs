...
impl Filter {
    pub fn new(
        ...
        if config_origin.freshly_minted() {
            log::info!(
                "config freshly minted, so we're assuming a brand new project; using `WildWest` filtering strategy"
            );
            Ok(Self::WildWest)
        } else if dot_first_init_exists {
            log::info!(
                "`{}` exists, so we're assuming a brand new project; using `WildWest` filtering strategy",
                crate::init::DOT_FIRST_INIT_FILE_NAME
            );
            Ok(Self::WildWest)
        } else {
            log::info!(
                "existing config loaded, so we're assuming an existing project; using `Protected` filtering strategy"
            );
            ...
            if unprotected.is_empty() {
                log::warn!(
                    "no ignore entries were parsed from {:?}; project generation will more or less be a no-op",
                    gitignore_path
                );
            } else {
                log::info!(
