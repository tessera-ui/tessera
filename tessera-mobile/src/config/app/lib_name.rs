...
impl Display for Invalid {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NotAscii { app_name, .. } => write!(f, "\"{app_name}\" isn't valid ASCII.",)?,
            Self::StartsWithDigit { app_name, .. } => {
                write!(f, "\"{app_name}\" starts with a digit.")?
            }
            ...
            Self::ReservedWindows { app_name } => {
                write!(f, "\"{app_name}\" is a reserved name on Windows.",)?
            }
            Self::ReservedArtifacts { app_name } => {
                write!(f, "\"{app_name}\" is reserved by Cargo.",)?
            }
            Self::NotAlphanumericOrUnderscore {
                app_name,
                naughty_chars,
                ..
            } => write!(
                ...
                match is_reserved(app_name.deref()) {
                    Ok(()) => {
                        if app_name.chars().all(char_allowed) {
                            ...
                            Err(Invalid::NotAlphanumericOrUnderscore {
                                app_name: app_name.to_owned(),
                                naughty_chars,
                                suggested: None,
                            })
