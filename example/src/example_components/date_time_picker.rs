use tessera_components::{
    ...
    date_picker::{
        CalendarDate, DatePickerArgs, DatePickerDialogArgs, DatePickerState, date_picker,
        date_picker_dialog,
    },
    ...
    theme::MaterialTheme,
    time_picker::{
        DayPeriod, TimePickerArgs, TimePickerDialogArgs, TimePickerState, time_picker,
        time_picker_dialog,
    },
};
...
pub fn date_time_picker_showcase() {
    ...
    let dialog_args = DialogProviderArgs::new(move || {
    ...
    .dialog_content(move || match active_dialog.get() {
        PickerDialog::Date => {
            date_picker_dialog(
            ...
            lazy_column(
                &LazyColumnArgs::default()
                    ...
                    .content(move |scope| {
                        ...
                        scope.item(move || {
                            date_picker(&DatePickerArgs::default().state(date_state));
                        });

                        scope.item(|| spacer(&SpacerArgs::new(Modifier::new().height(Dp(24.0)))));
                        ...
                        scope.item(move || {
                            time_picker(&TimePickerArgs::default().state(time_state));
                        });
                            ...
                            row(RowArgs::default(), move |row_scope| {
                                row_scope.child(move || {
                                    button(&ButtonArgs::with_child(
                                        ...
                                        || text(&TextArgs::from("Open date dialog")),
                                    ));
                                });
