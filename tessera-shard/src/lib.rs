...
///
/// 1. Application: ShardState exists for the lifetime of the application and
///    will not be destroyed.
/// 2. Shard: ShardState's lifecycle matches the navigation target, meaning it
///    will be destroyed when the page is popped.
#[derive(Debug, PartialEq, Eq)]
pub enum ShardStateLifeCycle {
    /// ShardState exists for the lifetime of the application and will not be
    /// destroyed.
    Application,
    /// ShardState's lifecycle matches the navigation target, meaning it will be
    /// destroyed when the page is popped.
    Shard,
}
...
impl ShardRegistry {
    ...
    /// Get or initialize and get a shard state, and provide it to the closure
    /// `f` as `Arc<T>`. The state type must implement `ShardState`.
    ///
    /// This function should never be called manually; it should be
    /// automatically generated by the `#[shard]` macro.
    ///
    /// # Safety
    ///
    /// This function is unsafe because it uses an evil method to cast `Arc<dyn
    /// ShardState>` to `Arc<T>`.
